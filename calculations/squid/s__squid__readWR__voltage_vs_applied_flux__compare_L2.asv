...
%% input parameters
%from WRSpice
p.fileNames = {'_dat__squid__Ic_20uA__Ib_40uA__L1_500pH__L2_10pH__beta_c_0p3',...
               '_dat__squid__Ic_20uA__Ib_40uA__L1_500pH__L2_15pH__beta_c_0p3',...
               '_dat__squid__Ic_20uA__Ib_40uA__L1_500pH__L2_20pH__beta_c_0p3',...
               '_dat__squid__Ic_20uA__Ib_40uA__L1_500pH__L2_25pH__beta_c_0p3',...
               '_dat__squid__Ic_20uA__Ib_40uA__L1_500pH__L2_30pH__beta_c_0p3',...
               };
...
figureCaptions = {sprintf('JJ I_c = %g uA',I_c*1e6),...
    ...
    sprintf('L_{parasitic} = %g pH',L_parasitic*1e12),...
    sprintf('SQUID beta_L = [%g %g %g %g %g]',2*I_c*(2*L2_vec(1)+L_parasitic)/p.Phi0,2*I_c*(2*L2_vec(2)+L_parasitic)/p.Phi0,2*I_c*(2*L2_vec(3)+L_parasitic)/p.Phi0,2*I_c*(2*L2_vec(4)+L_parasitic)/p.Phi0,2*I_c*(2*L2_vec(5)+L_parasitic)/p.Phi0),...
    sprintf('factor of 2 in M comes from two MIs for symmetry'),...
    };
    
figure('OuterPosition',[0 0 scrsz(3) scrsz(4)]);
plot(mutual_inductance_vec(1)*avg_current_vec{1}/p.Phi0,avg_voltage_vec{1}*1e6,'Color',bRGY(1,:),'LineStyle','-','LineWidth',2)
hold on
plot(mutual_inductance_vec(2)*avg_current_vec{2}/p.Phi0,avg_voltage_vec{2}*1e6,'Color',bRGY(2,:),'LineStyle','-','LineWidth',2)
plot(mutual_inductance_vec(3)*avg_current_vec{3}/p.Phi0,avg_voltage_vec{3}*1e6,'Color',bRGY(3,:),'LineStyle','-','LineWidth',2)
plot(mutual_inductance_vec(4)*avg_current_vec{4}/p.Phi0,avg_voltage_vec{4}*1e6,'Color',bRGY(4,:),'LineStyle','-','LineWidth',2)
plot(mutual_inductance_vec(5)*avg_current_vec{5}/p.Phi0,avg_voltage_vec{5}*1e6,'Color',bRGY(5,:),'LineStyle','-','LineWidth',2)
xlabel('Applied Flux [\Phi/\Phi_0]','FontSize',fontSize,'FontName','Times')
ylabel('Time-averaged SQUID Voltage [\mu V]','FontSize',fontSize,'FontName','Times')
legend('L_2 = 10 pH','L_2 = 15 pH','L_2 = 20 pH','L_2 = 25 pH','L_2 = 30 pH')
title('Comparing SQUID response for several values of \beta_L')
set(gca,'FontSize',fontSize,'FontName',fontName)
grid on
k1 = gtext(figureCaptions(1:length(figureCaptions)));
set(k1,'FontSize',fontSize_legend,'FontName','Times')
