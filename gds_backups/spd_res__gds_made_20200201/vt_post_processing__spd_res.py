...
from nc_library__vt_util import vt_fill
...
def vt_data_prep(D_in, new_device_name = 'spd_res', params = params):
    ...
    t_fill = time.time()
    fill_exclude_layers_list = ['m1','m3','stf','stfp','r1','r1p','r2','r2p']
    layer_list = []
...
#    fill_region_m1 = pg.boolean(fill_region_m1,pg.offset(fill_exclude_region,distance = 0*params['fill_w_delta'], join_first = True, precision = precision, layer = layers['dp']),'A-B',precision = precision,layer = layers['m0f'])            
    ...
    print('m1/gp...')
    t_m1 = time.time()
    m1 = pg.extract(D_in,[layers['m1'],layers['m1e'],layers['m3']])
    m1p = pg.extract(D_in,[layers['m1p']])
    m1_outline = pg.outline(m1, distance = params['ground_plane_buffer'], precision = precision, layer = layers['dp'])
    m1_outline = pg.boolean(m1_outline,m1p,'A-B',precision = precision,layer = layers['m1'])
 
    m2 = pg.extract(D_in,[layers['m2']])
    m2 = pg.boolean(m2,m1,'A-B',precision = precision,layer = layers['m1'])
    stf = pg.extract(D_in,[layers['stf']])
    m2 = pg.boolean(m2,stf,'A-B',precision = precision,layer = layers['m1'])
    m2 = pg.boolean(m2,m1_outline,'A+B',precision = precision,layer = layers['m1'])
    stf_outset = pg.offset(stf, distance = params['ground_plane_buffer'], precision = precision, layer = layers['dp'])
    stfp = pg.extract(D_in,[layers['stfp']])
    stf_outset = pg.boolean(stf_outset,stfp,'A-B',precision = precision,layer = layers['dp'])
    rs = pg.extract(D_in,[layers['r1'],layers['r2']])
    rs_outset = pg.offset(rs, distance = params['ground_plane_buffer'], precision = precision, layer = layers['dp'])
    cntcs = pg.extract(D_in,[layers['m1'],layers['m3'],layers['stf'],layers['r1p'],layers['r2p']])
    rs_outset = pg.boolean(rs_outset,cntcs,'A-B',precision = precision,layer = layers['dp'])
    m2 = pg.boolean(m2,stf_outset,'A+B',precision = precision,layer = layers['m1'])
    m2 = pg.boolean(m2,rs_outset,'A+B',precision = precision,layer = layers['m1'])
    ...
    m2 = pg.boolean(m2,D_holes_gp,'A+B',precision = precision,layer = layers['m1'])
    
    if params['post_processing_elim_small_features'] == True:
        D_out.add_ref(vt_eliminate_small_features(m2,params['post_processing_min_dim'],precision,layers['m1']))
    else:
        D_out.add_ref(m2)
